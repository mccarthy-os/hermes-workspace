...
export const Route = createFileRoute('/api/mccarthy/install-skill')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!rateLimit(`install-skill:${getClientIp(request)}`, 10, 60_000)) {
        ...
        const scriptPath = join(homedir(), 'mccarthy-os-template/scripts/install-skill.sh')
        const uid = parseInt(
        ...
        try {
          ...
          return json({ ok: true, message: `Installed ${skillId} for ${username}` })
        } catch (err) {
